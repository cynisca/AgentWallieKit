...
struct FeatureListComponentView: View {
    ...
    private var cardPerItem: Bool { data.props.cardPerItem == true }

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ForEach(Array(data.props.items.enumerated()), id: \.offset) { _, item in
                itemRow(item)
                    .applyOptionalBackground(cardPerItem ? rowBackgroundColor : nil)
                    .applyOptionalCornerRadius(cardPerItem && rowCornerRadius > 0 ? rowCornerRadius : nil)
                    .applyIf(cardPerItem, transform: { view in
                        view.applyOptionalBorder(
                            color: resolveColor(data.style?.borderColor, theme: theme),
                            width: data.style?.borderWidth,
        ...
        .applyIf(!cardPerItem) { view in
            view.applyOptionalBackground(rowBackgroundColor)
    ...
    private func itemRow(_ item: FeatureListComponentData.FeatureItem) -> some View {
        HStack(spacing: 12) {
            if !cardPerItem, let barColor = accentBarColor {
